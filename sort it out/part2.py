...
import threading as t  # used in the second portion of the program
...
def getComic(comicNumber: int) -> None:
    ...
    try:
        ...
        with open(f"comic{comicNumber}.{comicImageUrl[-3:]}",
                  "wb") as outputfile:  # assuming 3-char image extension only
...
start = time.time()
# Your code comes below here
threads = []
for num in list2:
    thread = t.Thread(target=getComic, args=(num,))
