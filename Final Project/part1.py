...
class Gui():
    ...
    def __init__(self):
        ...
        self.preyIcon = self.canvas.create_rectangle(
            (40, 40), (50, 50), fill="blue", outline="blue")
...
class Game():
    ...
    def move(self) -> None:
        ...
        self.queue.put({"move": self.snakeCoordinates}) # add a "move" task to the queue
        self.isGameOver(NewSnakeCoordinates) # check if the game is over
    
        overlap_x = prey_coords[0] <= NewSnakeCoordinates[0] <= prey_coords[2]  # check for prey being eaten
        overlap_y = prey_coords[1] <= NewSnakeCoordinates[1] <= prey_coords[3]
        
        if overlap_x and overlap_y: # if the prey is eaten
            self.score += 1 # increment the score
            newHeadCoords = self.calculateNewCoordinates() # find the new head position to increase snake
            self.snakeCoordinates.append(newHeadCoords) # append the new head position
            self.queue.put({"score": self.score}) # add a "score" task to the queue
            self.queue.put({"move": self.snakeCoordinates}) # add a"move" task to the queue to show the new snake
    ...
    def createNewPrey(self) -> None:
        ...
        while not valid_prey_coords:
            x = random.randint(x_min, x_max) #generate random x integer
            y = random.randint(y_min, y_max) #generate random y integer
            global prey_coords 
